...
class AgentLoop:
    ...
    def _register_default_tools(self) -> None:
        ...
        if getattr(self.tools_config.image_gen, "enabled", False):
            from nanobot.agent.tools.image_gen import ImageGenTool

            cfg = self.tools_config.image_gen
            self.tools.register(
                ImageGenTool(
                    api_key=cfg.api_key,
            ...
            registered.append("image_gen")

        if getattr(self.tools_config.video_gen, "enabled", False):
            from nanobot.agent.tools.video_gen import VideoGenTool

            cfg = self.tools_config.video_gen
            self.tools.register(
                VideoGenTool(
                    api_key=cfg.api_key,
                    base_url=cfg.base_url,
    ...
    async def _run_agent_loop(
        ...
        *on_stream_end(resuming)*: called when a streaming session finishes.
        ...
        loop_hook = AgentProgressHook(
            ...
            response_filter_tags=self.response_filter_tags,
        ...
        if result.final_content and self.response_filter:
            result.final_content = self.response_filter(result.final_content)
