...
_FLOAT_CAMEL_ALIASES: dict[str, str] = {
    "merge_window_s": "mergeWindowS",
}
...
class ChannelManager:
    ...
    def _init_channels(self) -> None:
        ...
        for name, cls in discover_enabled(enabled_names, _names=names).items():
            ...
            try:
                ...
                channel._merge_window_s = self._resolve_float_override(
                    section, "merge_window_s", self.config.channels.merge_window_s,
                )
    ...
    def _resolve_float_override(self, section: Any, key: str, default: float) -> float:
        """Return numeric *key* from *section* if present, otherwise *default*."""
        if isinstance(section, dict):
            value = section.get(key)
            if value is None:
                camel = _FLOAT_CAMEL_ALIASES.get(key)
                if camel:
                    value = section.get(camel)
        else:
            value = getattr(section, key, None)
