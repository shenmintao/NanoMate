...
def strip_think(text: str, extra_tags: list[str] | None = None) -> str:
    ...
    *extra_tags* allows callers to supply additional tag names (e.g. from
    ...
    for tag in extra_tags or []:
        if tag in ("think", "thought"):
            continue
        escaped = re.escape(tag)
        text = re.sub(rf"<{escaped}>[\s\S]*?</{escaped}>", "", text)
        text = re.sub(rf"^\s*<{escaped}>[\s\S]*$", "", text)
    return text.strip()
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    if tpl_skills.is_dir():
        for src in tpl_skills.rglob("*"):
            if src.is_file() and src.name != "__init__.py":
                _write(src, workspace / "skills" / src.relative_to(tpl_skills))
