...
if TYPE_CHECKING:
    ...
    from nanobot.agent.tools.life_action import LifeActionToolConfig
...
class ChannelsConfig(Base):
    ...
    merge_window_s: float = 0  # Merge messages from the same sender within N seconds (0 = disabled)
...
class ImageGenConfig(Base):
    """Legacy NanoMate image generation tool configuration."""
...
class VideoGenConfig(Base):
    """Legacy NanoMate video generation tool configuration."""
...
class SillyTavernConfig(Base):
    ...
    enabled: bool = False
    response_filter_tag: list[str] | str | None = None
...
class ToolsConfig(Base):
    ...
    life_action: LifeActionToolConfig = Field(
        default_factory=lambda: _lazy_default("nanobot.agent.tools.life_action", "LifeActionToolConfig"),
        alias="lifeAction",
    )
...
def _resolve_tool_config_refs() -> None:
    ...
    mod.LifeActionToolConfig = LifeActionToolConfig  # type: ignore[attr-defined]
