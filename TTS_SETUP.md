...
1. 已在 NAS 上部署 GPT-SoVITS（并完成模型训练）
2. GPT-SoVITS 服务运行在 `http://192.168.10.50:9880`（示例地址）
3. **准备参考音频**：即使训练完成，推理时仍需要 3-10 秒的参考音频来指定音色

> **重要**：GPT-SoVITS 采用 Few-Shot 克隆技术，**每次合成都需要参考音频**。
> 参考音频不是用来训练，而是告诉模型"用这个声音来合成"。
>
> 建议：准备一个固定的高质量参考音频，放在 NAS 固定位置，配置一次即可长期使用。
