...
def _response_filter_tags(raw: list[str] | str | None) -> list[str]:
    if not raw:
        return []
...
def _attach_sillytavern_hooks(agent: AgentLoop, config: Config) -> None:
    """Attach optional SillyTavern context and response filtering hooks."""
    try:
        st_cfg = config.sillytavern
    except Exception:
        return
    if not getattr(st_cfg, "enabled", False):
        return

    def _build_st_context() -> str:
        try:
            from nanobot.sillytavern.character_card import build_character_prompt
            from nanobot.sillytavern.preset import (
                apply_macros,
                build_preset_prompt,
                get_enabled_prompts,
            )
            from nanobot.sillytavern.storage import (
                get_active_character,
                get_active_preset,
                get_enabled_world_info,
            )
            from nanobot.sillytavern.types import MacrosConfig, WorldInfoBook
            ...
            if wi_books:
                entries = []
                for book in wi_books:
                    entries.extend(get_activated_entries(WorldInfoBook(**book), ""))
                wi_prompt = build_world_info_prompt(entries)
                if wi_prompt:
            ...
            if preset:
                ...
                if preset_prompt:
                    parts.append(apply_macros(preset_prompt, MacrosConfig()))

            if not parts:
                return ""
            return "# SillyTavern\n\n" + "\n\n---\n\n".join(parts)
        except Exception:
            logger.opt(exception=True).debug("SillyTavern context hook failed")
            return ""

    agent.context.context_hook = _build_st_context

    tags = _response_filter_tags(st_cfg.response_filter_tag)
    if not tags:
        return

    def _filter(content: str) -> str:
        filtered = content
        for tag in tags:
            escaped = re.escape(tag)
            filtered = re.sub(rf"<{escaped}>.*?</{escaped}>", "", filtered, flags=re.DOTALL)
        return filtered.strip() or content

    agent.response_filter = _filter
    agent.response_filter_tags = tags
    message_tool = agent.tools.get("message")
    if message_tool is not None and hasattr(message_tool, "response_filter_tags"):
        message_tool.response_filter_tags = tags
...
def serve(
    ...
    _attach_sillytavern_hooks(agent_loop, runtime_config)
